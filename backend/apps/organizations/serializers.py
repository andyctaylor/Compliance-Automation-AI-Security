...
class OrganizationRegistrationSerializer(serializers.Serializer):
    ...
    def create(self, validated_data):
        ...
        user = User.objects.create_user(
            ...
            job_title=validated_data['jobTitle'] if hasattr(User, 'job_title') else None,
            mobile_phone=validated_data['mobilePhone'] if hasattr(User, 'mobile_phone') else None,
            security_question=validated_data['securityQuestion'] if hasattr(User, 'security_question') else None,
            security_answer_hash=security_answer_hash if hasattr(User, 'security_answer_hash') else None,
        )
...
# --- Organization App Serializers (added to satisfy imports in viewsets) ---

class OrganizationMembershipSerializer(serializers.ModelSerializer):
    """Serializer for organization memberships (user-role within an org)."""
...
class OrganizationSerializer(serializers.ModelSerializer):
    """Read serializer for Organization with computed fields."""
...
class OrganizationCreateSerializer(serializers.ModelSerializer):
    ...
    def validate_slug(self, value):
        # Basic slug validation; DRF's SlugField on model enforces format.
    ...
    def create(self, validated_data):
        ...
        # Optionally, make the creator a member/admin if request present
        request = self.context.get('request')
        if request and request.user and request.user.is_authenticated:
